...
The combat screen presents the strain combat system on a mobile-first vertical layout. Shows player stats, enemies, and action decisions in a single downward flow with per-entity floating number animations, sequential execution replay, and a compact battle log.
