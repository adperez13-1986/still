...
When both actions in a linked pair are pushed, a synergy effect activates based on the type combination of the two actions. Synergies add bonus effects (damage, healing, block conversion, etc.) on top of the actions firing normally. The link tax (+1 strain) is charged when the synergy actually fires.
